...
const ChangePasswordModal = ({open, onClose}) => {
  ...
  return (
    ...
    justify-center p-4'>
      ...
      w-full max-w-md animate-fade-in' onClick={(e) => e.stopPropagation()}>
        <div className='flex items-center justify-between p-6 pb-0'>
            <h2 className='text-lg font-medium text-slate-900 
            flex items-center gap-2'><LockIcon className="w-5 h-5 text-slate-400"/> Change Password
            </h2>
  ...
  message.type === "success" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" 
    : "bg-rose-50 text-rose-700 border border-rose-200"}`}>
                    <div className={`w-1.5 h-1.5 rounded-full mt-1.5 
                        shrink-0 ${message.type === "success" ? "bg-emerald-500" : 
                            "bg-rose-500"}`}/>
                    {message.text}
                    </div>
            )}
            ...
            <div className='flex gap-3 pt-2'>
                <button className="btn-secondary flex-1"
                 ...
                 <button className="btn-primary flex-1 flex 
