...
import { Loader2, LogOutIcon, MenuIcon, UserIcon, XIcon } from 'lucide-react';
import { 
  LayoutGridIcon, 
  UsersIcon, 
  CalendarIcon, 
...
const Sidebar = () => {
  ...
  const sidebarContent = (
    <>
      <div className='px-5 pt-6 pb-5 border-b border-white/6'>
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              <p className='font-semibold text-[15px] text-white tracking-wide'>Employee MS</p>
              <p className='text-[12px] text-slate-500 font-medium'>Management System</p>
            </div>
          </div>
          ...
          navItems.map((item) => {
            ...
            return ( 
              <Link key={item.name} to={item.href}
                className={`group flex items-center gap-3 px-4 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/15 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/5"}`}>
      ...
      <div className='p-3 border-t border-white/6'> 
        <button onClick={handleLogout} className='flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-all duration-150'>
  ...
  return (
    <>
      ...
      <aside className='hidden lg:flex flex-col h-full w-[260px] bg-slate-900 text-white shrink-0 border-r border-white/5'>
        {sidebarContent}
      </aside>

      <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-slate-900 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        {sidebarContent}
      </aside>
    </>
  );
